...
public class DangerSmokeTests : IDisposable
{
    ...
    private (int exitCode, string stdout, string stderr) RunCli(string args)
    {
        ...
        {
            FileName = "dotnet",
            Arguments = $"run --project {Path.Combine(_repoRoot, "src", "Agentsmd", "Agentsmd.csproj")} -- {args} --project \"{_tempDir}\"",
    ...
    public void SourcesRegistry_Default_IsUsableForInstall()
    {
        ...
        var installed = File.ReadAllText(Path.Combine(_tempDir, ".agentsmd", "installed.json"));
