...
""";
    ...
    public static Command CreateMemoryCommand()
    {
        var cmd = new Command("memory", "Full-text search index for project memory (ADRs, changelog, known issues)");

        cmd.AddCommand(CreateMemoryIndexCommand());
        cmd.AddCommand(CreateMemorySearchCommand());
        cmd.AddCommand(CreateMemoryStatsCommand());
        cmd.AddCommand(CreateMemoryRecentCommand());

        return cmd;
    }

    private static Command CreateMemoryIndexCommand()
    {
        var cmd = new Command("index", "Scan project memory files and (re)build the full-text search index");
        cmd.SetHandler((string project) =>
        {
            var state = new InstalledStateManager(project);
            using var db = new MemoryDb(state.MemoryDbPath);
            var count = MemoryIndexer.IndexProject(db, project);
            AnsiConsole.MarkupLine($"[{Cli.Success}]Indexed {count} entries[/]");
            AnsiConsole.MarkupLine($"[{Cli.Muted}]{state.MemoryDbPath}[/]");
        }, ProjectOption);
        return cmd;
    }

    private static Command CreateMemorySearchCommand()
    {
        var queryArg = new Argument<string>("query", "FTS5 keyword search query");
        var limitOpt = new Option<int>(["--limit", "-n"], () => 5, "Maximum results to return");
        var cmd = new Command("search", "Search the project memory index") { queryArg };
        cmd.AddOption(limitOpt);

        cmd.SetHandler((string query, int limit, string project) =>
        {
            var state = new InstalledStateManager(project);
            if (!File.Exists(state.MemoryDbPath))
            {
                Console.Error.WriteLine("No memory index found. Run 'linxmd memory index' first.");
                return;
            }
            ...
            {
                var snippet = r.Snippet.Length > 80 ? r.Snippet[..77] + "..." : r.Snippet;
                var title = r.Title.Length > 55 ? r.Title[..52] + "..." : r.Title;
                table.AddRow(
                    $"[{Cli.Muted}]{Markup.Escape(r.Type)}[/]",
                    $"[{Cli.Primary}]{Markup.Escape(title)}[/]",
                    Markup.Escape(snippet),
                    $"[{Cli.Muted}]{Markup.Escape(r.SourcePath)}[/]");
            }
            AnsiConsole.Write(table);
        }, queryArg, limitOpt, ProjectOption);

        return cmd;
    }

    private static Command CreateMemoryStatsCommand()
    {
        var cmd = new Command("stats", "Show counts of indexed entries by type");
        cmd.SetHandler((string project) =>
        {
            var state = new InstalledStateManager(project);
            if (!File.Exists(state.MemoryDbPath))
            ...
            if (stats.Count == 0)
            {
                Console.WriteLine("Index is empty. Run 'linxmd memory index' to populate it.");
                return;
            }

            foreach (var (type, count) in stats.OrderBy(kv => kv.Key))
                AnsiConsole.MarkupLine($"  [{Cli.Primary}]{type,-15}[/] [{Cli.Muted}]{count}[/]");
        }, ProjectOption);
        return cmd;
    }

    private static Command CreateMemoryRecentCommand()
    {
        var typeOpt = new Option<string?>("--type", "Filter by type: decision, changelog, issue");
        var limitOpt = new Option<int>(["--limit", "-n"], () => 10, "Maximum results to return");
        var cmd = new Command("recent", "List most recently indexed entries");
        cmd.AddOption(typeOpt);
        cmd.AddOption(limitOpt);

        cmd.SetHandler((string? type, int limit, string project) =>
        {
            var state = new InstalledStateManager(project);
            if (!File.Exists(state.MemoryDbPath))
            {
            ...
            using var db = new MemoryDb(state.MemoryDbPath);
            var results = db.Recent(type, limit);
            if (results.Count == 0)
            {
            ...
            foreach (var r in results)
            {
                var title = r.Title.Length > 60 ? r.Title[..57] + "..." : r.Title;
                table.AddRow(
                    $"[{Cli.Muted}]{Markup.Escape(r.Type)}[/]",
                    $"[{Cli.Primary}]{Markup.Escape(title)}[/]",
                    $"[{Cli.Muted}]{Markup.Escape(r.CreatedAt)}[/]",
                    $"[{Cli.Muted}]{Markup.Escape(r.SourcePath)}[/]");
            }
            AnsiConsole.Write(table);
        }, typeOpt, limitOpt, ProjectOption);
